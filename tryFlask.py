...
from transformer_encoder.layers import WordEmbedding, PositionalEncoding, Dense, MultiHeadAttention, LayerNormalization
from transformer_encoder.activation import GeLu, Linear
...
def analyze_bow():
    try:
        ...
        logging.info("/analyze_bow was accessed.")
        ...
        # Check if jobDescription is empty or None
        if not job_description:
            logging.warning("Job description is empty. Aborting analysis.")
        ...
        for rank, (idx, similarity) in enumerate(similarity_scores, start=1):
            ...
            logging.info(f"BoW: Processing resume {rank}/{len(similarity_scores)}...")

        # Measure time elapsed between program start and end
        end_time = time.time()
        elapsed_time = end_time - start_time
        logging.info(f"BoW processing time: {elapsed_time} seconds") 
...
tokenizer.load_vocab('transformer_encoder/vocab.npy')
...
model = Sequential([
            ...
            LayerNormalization(),
            # Layer 1
            MultiHeadAttention(num_heads, model_dim),
			LayerNormalization(),
			# Feed Forward Network
			Dense([model_dim, ffn_dim], GeLu),
...
model.load_weights('transformer_encoder/model_weights.npy')
...
def analyze_te():
    try:
        ...
        preprocessed_job_desc, attention_mask = tokenizer(job_description, pad_token='[PAD]', mask_token='[MASK]', oov_token='[UNK]', max_pos=512)

        job_desc_te = model.predict(preprocessed_job_desc, attention_mask)
        job_desc_te = np.mean(job_desc_te, axis=0, keepdims=True)

        resumes_te = []
        for index, resume in enumerate(resumes):
            logging.info(f"TE: Processing resume {index + 1}/{len(resumes)}")
            preprocessed_resume, attention_mask = tokenizer(resume, pad_token='[PAD]', mask_token='[MASK]', oov_token='[UNK]', max_pos=512)
            resume_embedding = model.predict(preprocessed_resume, attention_mask)
            resumes_te.append(np.mean(resume_embedding, axis=0, keepdims=True))

        # Calculate cosine similarity between job description and each resume
        similarities = []
        for index, resumes_te_embedding in enumerate(resumes_te):
            logging.info(f"CS: Processing resume {index + 1}/{len(resumes_te)}")
            similarities.append(cosine_similarity(job_desc_te, resumes_te_embedding)[0][0])

        logging.info(len(similarities))
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        logging.info(f"TE processing time: {elapsed_time} seconds") 
