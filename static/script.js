  ...
  checkboxesTE.forEach(async (checkbox) => {
    if (checkbox.checked) {
      ...
      try {
        ...
        console.log(`File ${fileName} downloaded from TE folder`);
      } catch (error) {
        console.error(`Error downloading file ${fileName} from TE folder:`, error);
      }
    }
  });
  ...
  checkboxesBOW.forEach(async (checkbox) => {
    if (checkbox.checked) {
      ...
      try {
        ...
        downloadFile(url, fileName)
        console.log(`File ${fileName} downloaded from BOW folder`);
      } catch (error) {
        console.error(`Error downloading file ${fileName} from BOW folder:`, error);
      }
    }
  });
}
...
function displayFileInTable(file, tableBody) {
  ...
  const downloadURL = `https://firebasestorage.googleapis.com/v0/b/resume-checker-2.appspot.com/o/${storagePath}?alt=media`;
