...
class Optimizer(abc.ABC):
    ...
    def update_params(self,
                      dW: NDArray[np.float32],  # weight gradients
                      db: NDArray[np.float32],  # bias gradients
                      W: NDArray[np.float32],  # current weights
                      b: NDArray[np.float32]) -> Tuple[NDArray[np.float32], NDArray[np.float32]]:  # current biases
        pass
